...
public class dissolve1Script : MediaEffect {
    ...
    IEnumerator shaderEffect()
    {
        ...
        if (img)
        {
            ...
            while (timer > 0)
            {
                img.material.SetFloat("Vector1_FD5F235C", Mathf.Lerp(1.0f, 0.0f, timer / delayTimer));
        ...
        if (media)
        {
            ...
            while (timer > 0)
            {
                rend.material.SetFloat("Vector1_FD5F235C", Mathf.Lerp(1.0f, 0.0f, timer / delayTimer));
                yield return new WaitForSeconds(delta);
                timer -= delta;
            }
        }
