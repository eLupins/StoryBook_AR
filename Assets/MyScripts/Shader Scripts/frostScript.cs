...
public class frostScript : MediaEffect {
    ...
    IEnumerator shaderEffect()
    {
        ...
        if (img)
        {
            ...
            while (timer > 0)
            {
                img.material.SetFloat("Vector1_96DE319D", Mathf.Lerp(0.0f, 1.0f, timer / delayTimer));
        ...
        if (media)
        {
            ...
            while (timer > 0)
            {
                rend.material.SetFloat("Vector1_96DE319D", Mathf.Lerp(0.0f, 1.0f, timer / delayTimer));
                yield return new WaitForSeconds(delta);
                timer -= delta;
            }
        }
